...
for i in range(diaposon):
    ...
    while(1):
        number = myFaker.pyint(min_value=1, max_value=7, step=1)
        if (number not in communication_ids or len(communication_ids) >= 7):
            communication_ids.append(number)
